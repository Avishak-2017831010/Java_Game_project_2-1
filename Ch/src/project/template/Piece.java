...
public abstract class Piece extends Group{
	...
	protected int type;
	// positionition of the piece on the board
	...
	// Highlight all the available positionition where the piece can go
	public void SelectPiece(chessBoard board) {
	}
	
	// Move method: When a piece is already selected and that the player click on a highlighted positionition
	// Change the positionition of the piece and update the board
	public void MovePiece(chessBoard chessBoard, int x, int y) {
		chessBoard.setBoard(this.xposition, this.yposition, 0);
		chessBoard.setPiece(this.xposition, this.yposition, null);
		if (!chessBoard.checkState && this.canCastle(chessBoard)!=0){
			if(this.canCastle(chessBoard)==1){
				...
				chessBoard.getPiece(7, y).xposition = 7;
			...
			if(this.canCastle(chessBoard)==2){
				...
				chessBoard.getPiece(3, y).xposition = 3;
			...
			if(this.canCastle(chessBoard)==3){
				...
				chessBoard.setBoard(5, y, chessBoard.getPiece(7, y).type);
				chessBoard.setPiece(5, y, chessBoard.getPiece(7, y)); 
				chessBoard.getPiece(5, y).xposition = 5;
				chessBoard.setBoard(7, y, 0);
				chessBoard.setPiece(7, y, null);
			}
			...
			if (this.name == "Pawn" && ((this.type == 1 && this.yposition == 0) || (this.type == 2 && this.yposition == 7)))
			{
	...
	public void capture(chessBoard chessBoard) {
		if (this.type == 1)
		{
			if (this.name == "Rook")
				...
				chessBoard.playerOnePawn--;
			else if (this.name == "Bishop" && (this.xposition + this.yposition) % 2 != 0)
